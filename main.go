...
func main() {
	...
	cleanup := func() {
		...
		// 2. Stop Xray process (with graceful shutdown)
		if xrayProc != nil {
			log.Println("[JhopanStoreVPN] Stopping Xray process...")
			if err := xrayProc.Stop(); err != nil {
				log.Printf("[JhopanStoreVPN] Xray stop error (non-fatal): %v", err)
			}
			xrayProc = nil
		}
	...
	onXrayCrash := func() {
		log.Println("[JhopanStoreVPN] Xray crashed")
		if pinger != nil {
			pinger.Stop()
		}
	...
	doConnect = func() {
		...
		if !xray.HasBinary() {
			dialog.ShowError(fmt.Errorf("xray.exe tidak ditemukan.\nLetakkan file xray.exe di folder yang sama dengan JhopanStoreVPN.exe"), w)
			mainPage.SetDisconnected()
			return
		}
		...
		go func() {
			...
			// Wait for xray SOCKS5 port to be ready with timeout feedback (up to 10 seconds)
			...
			if !portReady {
				xrayProc.Stop()
				mainPage.SetDisconnected()
				mainPage.SetStatus("Xray connection timeout (10s)")
				return
			}
			log.Println("[JhopanStoreVPN] Xray SOCKS5 port 10808 is ready")

			// TUN is handled directly by Xray inbound.
			// Xray config already includes TUN adapter setup
